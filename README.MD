...
Note that the time series are scaled internally by the algorithm, there is no need to scale the time series in advance.
...
- `hidden-size`: `int`. The number of units of the LSTM layer (default = 8).
